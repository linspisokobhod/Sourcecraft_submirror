...
class SourceCraftUploader:
    ...
    def get_user_repos(self) -> List[Dict]:
        ...
        try:
            # Правильный эндпоинт для получения репозиториев пользователя
    ...
    def get_repository(self, owner: str, repo: str) -> Optional[Dict]:
        ...
        try:
            # Правильный эндпоинт для получения репозитория
    ...
    def create_repository(self, name: str, description: str = "") -> Dict:
        ...
        # Правильный эндпоинт для создания репозитория
        url = f"{self.base_url}/user/repos"
        data = {
            "name": name,
    ...
    def get_file_contents(self, owner: str, repo: str, path: str) -> Optional[Dict]:
        ...
        try:
            # Правильный эндпоинт для получения содержимого файла
    ...
    def create_or_update_file(self, owner: str, repo: str, path: str, content: str, message: str, sha: Optional[str] = None) -> Dict:
        ...
        # Правильный эндпоинт для работы с содержимым файла
        url = f"{self.base_url}/repos/{owner}/{repo}/contents/{path}"
...
def get_api_key():
    ...
    # Согласно документации SourceCraft, правильное имя переменной - SOURCECRAFT_TOKEN [citation:1][citation:9]
    possible_names = [
        'SOURCECRAFT_TOKEN',
        'SOURCECRAFT_API_KEY',
    ...
    logger.error("❌ API ключ не найден. Используйте переменную SOURCECRAFT_TOKEN [citation:1]")
    return None
...
def main():
    ...
    # Проверка подключения к API - используем правильный эндпоинт
    try:
