...
class SourceCraftUploader:
    ...
    def get_user_info(self) -> Optional[Dict]:
        ...
        try:
            ...
            logger.info(f"🔍 Запрос информации о пользователе: {url}")
            response = requests.get(url, headers=self.headers, timeout=30)
            response.raise_for_status()
            result = response.json()
            logger.info(f"✅ Пользователь: {result.get('login', 'unknown')}")
            return result
    ...
    def get_repository_by_name(self, username: str, repo_name: str) -> Optional[Dict]:
        ...
        try:
            ...
            if response.status_code == 200:
                repo = response.json()
                logger.info(f"✅ Найден репозиторий: {repo.get('name')}")
                return repo
            elif response.status_code == 404:
                logger.info(f"📦 Репозиторий '{repo_name}' не найден")
                return None
            else:
    ...
    def get_file_contents(self, username: str, repo_name: str, file_path: str) -> Optional[Dict]:
    ...
    def create_or_update_file(self, username: str, repo_name: str, file_path: str, content: str, message: str, sha: Optional[str] = None) -> Dict:
...
def main():
    ...
    try:
        test_url = f"{uploader.base_url}/me"
        response = requests.get(test_url, headers=uploader.headers, timeout=10)
        if response.status_code == 200:
            user_info = response.json()
            logger.info(f"✅ API доступен. Пользователь: {user_info.get('login', 'unknown')}")
        else:
    ...
    repo = uploader.get_repository_by_name(username, repo_name)
    ...
    repo_id = repo.get('id')
    logger.info(f"✅ Работа с репозиторием: {repo.get('name')} (ID: {repo_id})")
    logger.info(f"🔗 URL: https://sourcecraft.dev/{username}/{repo_name}")
    ...
    for sub in subscriptions:
        ...
        file_info = uploader.get_file_contents(username, repo_name, filename)
        existing_sha = file_info.get('sha') if file_info else None
        
        # Загрузка или обновление файла
        try:
            message = f"Update {filename} from source"
            ...
            logger.info(f"✅ {filename} загружен в репозиторий")
